...
uint8_t vsig_pot(uint16_t tvol, uint8_t vm)
{
    ...
    uint16_t mdiv7=572; //Pent(tmax/7)
    if(tvol>=0 && tvol<(mdiv7)){if(vm==0){return 0;}else if(vm>0){return vm;}}
...
uint8_t sel_nota(uint32_t dist, uint8_t mode)
{
    ...
    if(mode=1)
        {
            mdiv7=572; //(4000/7)
                if(dist>=0 && dist<(mdiv7)){return 1;}//Do4}
...
int main()
{
	...
	uint32_t rc_nota, rc_vol;
 	uint16_t b1, b2, b3, b4, boton;
	uint8_t c1, c2, c3, c4, c5, valor, note;
    int octava=0; //Defecto Octava4, =1 Octava4y5
...
gpio_init(0x003); //9BITS  6 ENTRADAS(BOTONES) 3 SALIDAS(2 LEDS Y PWM)
trigger_set(pitch); // 10us a 50 ms
...
for(;;)
    {
        while(boton!=PAUSE)
        {
            rc_nota=read_count(); //Obtiene el valor de tiempo de lectura del sensor
            note=sel_nota(rc_nota,mod);
            tone.t_mt=t_muestra(note);
            tone.sp=step(note);
            tone.n_mt=n_muestra(note);
            tone.t_fm=time_for(note);
            play_note(tone.n_mt, tone.sp, tone.t_mt, tone.t_fm);
